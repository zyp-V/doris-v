...
import org.apache.doris.thrift.TLegacyUpsertTxnStateRequest;
import org.apache.doris.thrift.TLegacyUpsertTxnStateResult;
...
public class FeServiceClient {
    ...
    private final int feThriftPort;
    private final String user;
    private final String password;
    private final int retryCount;
    private final int timeoutMs;
    private long consulAddressCacheTime = 0;
    private List<TNetworkAddress> cachedAddresses;
    ...
    public FeServiceClient(String name, List<TNetworkAddress> addresses,
            String psm, int feThriftPort, String user, String password,
            int retryCount, int timeoutMs) {
        this.name = name;
        this.addresses = addresses;
        this.psm = psm;
        this.feThriftPort = feThriftPort;
        this.user = user;
    ...
    private List<TNetworkAddress> getAddresses() {
        ...
        // get addresses from consul using psm
        // having race condition, but it doesn't matter
        if ((System.currentTimeMillis() - consulAddressCacheTime) < 5000) {
            return cachedAddresses;
        }
        Discovery discovery = new Discovery();
        List<TNetworkAddress> addresses = new ArrayList<>();
        List<ServiceNode> serviceInfo = discovery.lookupName(psm);
        for (ServiceNode item : serviceInfo) {
            addresses.add(new TNetworkAddress(item.getHost(), feThriftPort));
        }
        cachedAddresses = addresses;
        consulAddressCacheTime = System.currentTimeMillis();
        return addresses;
    }
    ...
    public static class TResultAdapter {
        public static TStatus getStatus(Object obj) {
            ...
            }  else if (obj instanceof TLegacyUpsertTxnStateResult) {
                return ((TLegacyUpsertTxnStateResult) obj).getStatus();
            }
        ...
        public static TNetworkAddress getMasterAddress(Object obj) {
            if (obj instanceof TBeginRemoteTxnResult) {
                ...
                return ((TLegacyUpsertTxnStateResult) obj).getMasterAddress();
            }
            throw new IllegalArgumentException("unsupported result type: " + obj.getClass().getName());
        }
    }
    ...
    public List<String> listDatabaseNames() {
        ...
        params.setCatalog(Env.getCurrentInternalCatalog().getName());
        params.setUser(user);
        params.setUserIp("%");
        String msg = String.format("failed to get all databases from remote doris:%s", name);
        return randomCallWithRetry(client -> {
            TGetDbsResult result = client.getDbNames(params);
            return result.getDbs();
        }, msg, timeoutMs);
    }
    ...
    public List<String> listTableNames(String dbName) {
        ...
        params.setUser(user);
        params.setUserIp("%");
        String msg = String.format("failed to get all tables from remote doris:%s, db:%s", name, dbName);
        return randomCallWithRetry(client -> {
    ...
    public RemoteOlapTable getOlapTable(String dbName, String table, long tableId, List<Partition> partitions,
        ...
        return randomCallWithRetry(client -> {
            ...
            try (DataInputStream in = new DataInputStream(new ByteArrayInputStream(result.getTableMeta()))) {
                OlapTable olapTable = OlapTable.readGSON(in);
    ...
    public boolean health() {
        ...
        request.setUserIp("%");
        request.setPasswd(password);
        String msg = String.format("failed to check auth for remote doris:%s", name);
        try {
            return randomCallWithRetry(client -> {
                TCheckAuthResult result = client.checkAuth(request);
                if (result.getStatus().getStatusCode() != TStatusCode.OK) {
                    LOG.warn("check remote doris:{} failed, msg:{}", name, result.getStatus().toString());
                }
                return result.getStatus().getStatusCode() == TStatusCode.OK;
            }, msg, timeoutMs);
        } catch (Exception e) {
            return false;
        }
    ...
    public void legacyUpsertUpsertTxnState(long transactionId, RemoteOlapTable table, boolean isPartialUpdate)
            throws UserException {
        TLegacyUpsertTxnStateRequest request = new TLegacyUpsertTxnStateRequest();
        request.setUser(user);
        request.setPasswd(password);
        request.setCatalog(table.getCatalog().getName());
        request.setDb(table.getDatabase().getFullName());
        request.setTbl(table.getName());
        request.setTxnId(transactionId);
        request.setIsPartialUpdate(isPartialUpdate);
        long startTime = System.currentTimeMillis();
        TLegacyUpsertTxnStateResult result;
        try {
            result = masterCallWithRetry(client -> client.legacyUpsertTxnState(request),
                    "failed to upsert legacy txn state to remote doris:" + name, timeoutMs);
        } catch (Exception e) {
            long costMs = System.currentTimeMillis() - startTime;
            LOG.warn("upsert legacy txn state failed, txn_id={},  cost={}ms", transactionId, costMs, e);
            throw new UserException(Util.getRootCauseMessage(e), e);
        }
        if (result.getStatus().getStatusCode() != TStatusCode.OK) {
            LOG.warn("upsert legacy txn state failed, txn_id={}, err={}", transactionId,
                    result.getStatus().getErrorMsgs().get(0));
