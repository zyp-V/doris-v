...
public class PaimonScanNode extends FileQueryScanNode {
    ...
    public List<org.apache.paimon.table.source.Split> getPaimonSplitFromAPI() throws UserException {
        ...
        try {
            ...
            LOG.warn("Failed to check latest paimon snapshot for table {}.{}, continue scanning",
                    targetTable.getDbName(), targetTable.getName(), e);
        }
        try {
            Table paimonTable = getProcessedTable();
            List<String> rowFieldNames = paimonTable.rowType().getFieldNames().stream()
            ...
            throw new UserException(throwable.getMessage(), throwable);
        }
    }

    @VisibleForTesting
    static boolean isPaimonSnapshotOutOfRangeException(Throwable throwable) {
        Throwable current = throwable;
        while (current != null) {
            String className = current.getClass().getName();
            if (className.contains("OutOfRangeException")
                    || className.contains("SnapshotNotExistException")
                    || containsSnapshotOutOfRangeMessage(current.getMessage())) {
                return true;
            }
    ...
    private static boolean containsSnapshotOutOfRangeMessage(String message) {
        if (message == null) {
            return false;
        }
        String lowerCaseMsg = message.toLowerCase(java.util.Locale.ROOT);
        return lowerCaseMsg.contains("out of available snapshotid range")
                || lowerCaseMsg.contains("snapshot does not exist")
                || (lowerCaseMsg.contains("scan snapshotid")
                        && lowerCaseMsg.contains("available snapshotid range"));
    }
    ...
    private static String getRootCauseMessage(Throwable throwable) {
        ...
        return rootCause.getMessage() == null ? throwable.toString() : rootCause.getMessage();
    }
