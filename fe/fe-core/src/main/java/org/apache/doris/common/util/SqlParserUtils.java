...
public class SqlParserUtils {
    ...
    public static StatementBase parseStmt(String originStmt, ConnectContext ctx) throws UserException {
        LOG.info("begin to parse stmt: " + originStmt);
        SqlScanner input = new SqlScanner(new StringReader(originStmt), ctx.getSessionVariable().getSqlMode());
        SqlParser parser = new SqlParser(input);
        StatementBase statementBase;
        try {
            ...
            if (stmts.size() > 1) {
                throw new AnalysisException("has two or more statements");
            }
    ...
    public static List<StatementBase> parseStmts(String originStmt, ConnectContext ctx)
