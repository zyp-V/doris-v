...
suite("test_query_remote_doris_as_olap_table_select", "p0,external,doris,external_docker,external_docker_doris") {
    ...
    qt_sql """
        select /*+ SET_VAR(enable_sql_cache=true) */ * from `${catalog_name}`.`${db_name}`.`test_remote_doris_all_types_select_t` order by id
    """

    qt_sql """
        select /*+ SET_VAR(enable_sql_cache=true) */ * from `${catalog_name}`.`${db_name}`.`test_remote_doris_all_types_select_t2` order by id
    """

    qt_sql """
        select /*+ SET_VAR(enable_sql_cache=true) */ * from `${catalog_name}`.`${db_name}`.`test_remote_doris_all_types_select_t3` order by id
    """
    ...
    // test insert into
    explain {
        sql("insert into `${db_name}`.`test_remote_doris_all_types_insert` select /*+ SET_VAR(enable_sql_cache=true) */ * from `${catalog_name}`.`${db_name}`.`test_remote_doris_all_types_select_t`")
        contains("VOlapScanNode")
    }
    sql """
        insert into `${db_name}`.`test_remote_doris_all_types_insert` select /*+ SET_VAR(enable_sql_cache=true) */ * from `${catalog_name}`.`${db_name}`.`test_remote_doris_all_types_select_t`
    """
    qt_after_insert_cmd """
        select /*+ SET_VAR(enable_sql_cache=true) */ * from `${db_name}`.`test_remote_doris_all_types_insert` order by id
    """
    // test insert overwrite
    explain {
        sql("insert OVERWRITE table `${db_name}`.`test_remote_doris_all_types_insert` select /*+ SET_VAR(enable_sql_cache=true) */ * from `${catalog_name}`.`${db_name}`.`test_remote_doris_all_types_select_t`")
        contains("VOlapScanNode")
    }
    sql """
        insert OVERWRITE table `${db_name}`.`test_remote_doris_all_types_insert` select /*+ SET_VAR(enable_sql_cache=true) */ * from `${catalog_name}`.`${db_name}`.`test_remote_doris_all_types_select_t`
    """
    ...
    """
    qt_join """
        select /*+ SET_VAR(enable_sql_cache=true) */ * from `${db_name}`.`left_inner_table` a
    ...
    qt_join_partition """
        select /*+ SET_VAR(enable_sql_cache=true) */ * from `${db_name}`.`left_inner_table_partition` a
    ...
    """
    qt_join_unique """
        select /*+ SET_VAR(enable_sql_cache=true) */ * from `${db_name}`.`left_inner_unique` a
    ...
    sql """
     ...
     """
     qt_aggregate """
        select /*+ SET_VAR(enable_sql_cache=true) */ log_type, sum(count) as count from `${catalog_name}`.`${db_name}`.`right_remote_table_aggregate` group by log_type order by log_type
    """

    qt_join_aggregate """
        select /*+ SET_VAR(enable_sql_cache=true) */ b.`log_type`, sum(b.`count`) as count from `${db_name}`.`left_inner_unique` a
